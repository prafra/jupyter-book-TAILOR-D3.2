...
On a different dimension, a lot of effort has been put into defining what are the possible  {doc}`./T3.1/XAI_dimensions` (e.g., we can discriminate between {doc}`./T3.1/model_specific`), the requirements to provide good explanations (see <!-- TODO: add anchor {ref}`guidelines` --> guidelines), how to <!-- TODO: add link [evaluate explanations](./T3.1/evaluating_explanations.md) --> evaluate explanations, and to understand the {doc}`./T3.1/feature_importance`. Then, it is important to note that a variety of different kinds of explanations can be provided, such as {doc}`./T3.1/single_tree`, {doc}`./T3.1/feature_importance`, {doc}`./T3.1/saliency_maps`, Factual and Counterfactual, Exemplars and Counter-Exemplars, and Rules List and Rules Sets.<!--[Factual and Counterfactual](./T3.1/counterfactuals.md), exemplars and counter-exemplars, [Rules List and Rules Sets](./T3.1/rules.md).-->
