...
according to some {doc}`./T3.5/L1.privacy_model`. Anonymization must no be reduced to {doc}`./T3.5/L2.pseudonymization` (see also {doc}`./T3.5/L2.reidentification`), which is defined in GDPR Article 4 as "the processing
...
This entry was written by Marie-Christine Rousset and Francesca Pratesi.
